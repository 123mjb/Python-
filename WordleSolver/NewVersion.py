...
def valuer(nonvalidletters,answers):
    ValidAnswers_value = answers
    nonvalid = []
    for p in range(0,len(ValidAnswers_value)):
        for o in ValidAnswers_value[p]:
            if o in nonvalidletters:
                nonvalid.append(p)
                break
    ...
    with open("WordleSolver/LetterPlaceValuesDuring.txt","w") as txtlv:
